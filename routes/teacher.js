  ...
  jwt.verify(token, process.env.JWT_SECRET, async (err, auth) => {
    ...
    if (auth.teacher_id) {
      ...
      let student = await db.query(
        `SELECT first_name, last_name, username FROM students WHERE student_id = '${result.rows[0].student_id}'`
      );
      res.status(200).json({
        result: result.rows[0],
        resultdata: resultdata.rows,
        test_name: test_name.rows[0].test_name,
...
router.get('/home', async (req, res) => {
  ...
  res.status(200).json({
    latestChangelog: changelog[changelog.length - 1],
...
router.post('/feedback', (req, res) => {
  let auth = res.locals.auth;
  db.query(
    `INSERT INTO feedback (submitted_by, feedback_type, feedback) VALUES ('${auth.teacher_id}', '${req.body.feedback_type}','${req.body.feedback}')`,
    (err, data) => {
      if (err) {
        console.log(err);
        return res.status(500).json(err);
