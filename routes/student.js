...
router.get('/testId', async (req, res) => {
  ...
  if (activeTest.rows[0].active_test === null) return res.status(200).json({test_id: null, first_name: auth.first_name})
    ...
    let allowedAttempts = await db.query(`SELECT attempts FROM tests WHERE test_id = '${activeTest.rows[0].active_test}'`)
    let attempts = await db.query(`SELECT * FROM results WHERE test_id = '${activeTest.rows[0].active_test}' AND student_id = '${auth.student_id}'`)
